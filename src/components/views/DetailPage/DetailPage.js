...
function DetailPage() {
    const item = data
    const orders = data.description
    const ListOrders = orders && orders.map((order) =>
        (<DetailCard className="detail" description={order}></DetailCard>)
    );
    return (
        <div className="page">
            ...
            <div className="paper">
                <div>
                    <Card className="content" title={item.title} channelTitle={item.channelTitle} thumbnails={item.thumbnails} description={item.description} ingredientsArr={item.ingredientsArr}></Card>
                </div>
                <div>
                    {ListOrders}
                </div>
